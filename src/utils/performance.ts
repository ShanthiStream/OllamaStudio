...
export function estimateModelPerformance(
  ...
  const memPenalty = memoryMB > availableRAM ? 0.5 : 1;
  ...
  if (memoryMB > availableRAM) recommendations.push(`Needs ~${formatBytes(memoryMB * 1024 * 1024)} RAM, but only ${formatBytes(availableRAM * 1024 * 1024 * 1024)} available`);
  ...
  return {
    ...
    score,
    label,
    fits_in_ram: memoryMB <= availableRAM,
    will_use_swap: memoryMB > availableRAM,
    recommendations,
  };
}
